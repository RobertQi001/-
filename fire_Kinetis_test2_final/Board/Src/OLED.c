...
const byte F14x16[] = 
{  
//0xFF,0xFF,0x07,0xEF,0xCD,0xAB,0xEF,0x07,0xAB,0xCD,0xEF,0x07,0xFF,0xFF,0xFF,0xFF,0xFE,0xC2,0xAA,0xAA,0xAA,0xAB,0xAA,0xAA,0x80,0xFE,0xFF,0xFF,/*"曾",0*/
/* (14 X 16 , 宋体 )*/

//0xFF,0x3F,0x0F,0xF3,0xFF,0x4F,0x87,0x6B,0xED,0xCF,0xA7,0x0F,0x7F,0xFF,0xFF,0xFF,0x80,0xFF,0xF6,0x7B,0xBC,0xD1,0xED,0xD1,0xBC,0xBF,0xBF,0xFF,/*"俊",1*/
/* (14 X 16 , 宋体 )*/

//0xFF,0xFF,0xEF,0xEF,0x6F,0x8F,0x03,0xEF,0x8F,0x6F,0xEF,0xF7,0xFF,0xFF,0xFF,0xBD,0x8D,0xFE,0xFF,0xCF,0xFC,0xFF,0x8F,0xFF,0xF6,0xCE,0xDE,0xFF/*"杰",2*/
/* (14 X 16 , 宋体 )*/
...
void SetAddressingMode(unsigned char d)//设置显示模式
{
...
void SetColumnAddress(unsigned char a, unsigned char b)//设置列地址
{
...
void SetPageAddress(unsigned char a, unsigned char b)//设置行地址
{
...
void SetStartLine(unsigned char d)//   设置开始显示的行
{
	LED_WrCmd(0x40|d);		// Set Display Start Line
							// Default => 0x40 (0x00)
}

void SetContrastControl(unsigned char d)   //设置对比度（默认0x7f,越大月亮）
{
	LED_WrCmd(0x81);			// Set Contrast Control
	LED_WrCmd(d);			// Default => 0x7F
}

void Set_Charge_Pump(unsigned char d)  //  电荷传送
{
...
void Set_Segment_Remap(unsigned char d)  //图像从0列开始填充显示
{
	LED_WrCmd(0xA0|d);		// Set Segment Re-Map
							// Default => 0xA0
							// 0xA0 (0x00) => Column Address 0 Mapped to SEG0
							// 0xA1 (0x01) => Column Address 0 Mapped to SEG127图像从127列开始填充显示
}

void Set_Entire_Display(unsigned char d)//根据RAM显示
{
	LED_WrCmd(0xA4|d);		// Set Entire Display On / Off
							// Default => 0xA4
							// 0xA4 (0x00) => Normal Display
							// 0xA5 (0x01) => Entire Display On  忽略RAM全屏显示，全亮
}

void Set_Inverse_Display(unsigned char d)//正常显示
{
	LED_WrCmd(0xA6|d);		// Set Inverse Display On/Off
							// Default => 0xA6
							// 0xA6 (0x00) => Normal Display
							// 0xA7 (0x01) => Inverse Display On  反白显示
}

void Set_Multiplex_Ratio(unsigned char d)//设置有效显示行数0~63
{
	LED_WrCmd(0xA8);			// Set Multiplex Ratio
	LED_WrCmd(d);			// Default => 0x3F (1/64 Duty)
}

void Set_Display_On_Off(unsigned char d)//关显示
{
	LED_WrCmd(0xAE|d);		// Set Display On/Off
							// Default => 0xAE
							// 0xAE (0x00) => Display Off
							// 0xAF (0x01) => Display On  开显示
}
...
void Set_Common_Remap(unsigned char d)//正常显示模式
{
...
void Set_Display_Offset(unsigned char d)//图像偏移设置
{
	LED_WrCmd(0xD3);			// Set Display Offset
	LED_WrCmd(d);			// Default => 0x00
}

void Set_Display_Clock(unsigned char d)//设置显示时钟振荡频率
{
...
void Set_Precharge_Period(unsigned char d)//设置预充电周期
{
...
void Set_Common_Config(unsigned char d)//硬件引脚配置
{
...
void Set_VCOMH(unsigned char d)   //设置管脚电压
{
