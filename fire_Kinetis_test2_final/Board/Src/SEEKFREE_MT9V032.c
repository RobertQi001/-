...
int16 MT9V032_CFG[CONFIG_FINISH][2]=
{
    ...
    {SET_COL,      CAMERA_W}, //图像列数量        范围1-752     K60采集不允许超过188
    {SET_ROW,      CAMERA_H}, //图像行数量        范围1-480
...
void mt9v032_init(uint8 imgaddr[MT9V032_H][MT9V032_W])
{
    ...
    dma_portx2buff_init(MT9V032_DMA_CH, (void *)&MT9V032_DATAPORT, (void *)imgaddr, MT9V032_PCLK, DMA_BYTE1, CAMERA_W*CAMERA_H, DADDR_KEEPON);
    port_init(MT9V032_PCLK, ALT1 | DMA_FALLING | PULLDOWN);  	//PCLK  触发源设置
    gpio_ddr (MT9V032_PCLK, GPI);    //设置 PTC18 管脚为输入
...
void mt9v032_vsync(void)
{
  ...
  dma_repeat(MT9V032_DMA_CH,(void *)&MT9V032_DATAPORT,(void *)image[0],CAMERA_W*CAMERA_H);
}
...
{
  //uart_putchar(VCAN_PORT,0x00);uart_putchar(VCAN_PORT,0xff);uart_putchar(VCAN_PORT,0x01);uart_putchar(VCAN_PORT,0x01);//发送命令
  //uart_putbuff(VCAN_PORT, (uint8_t *)image, CAMERA_W*CAMERA_H);  //发送图像
  vcan_sendimg((uint8_t *)image, CAMERA_W*CAMERA_H);                  //发送到上位机
}
