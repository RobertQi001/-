...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(OV7725_EAGLE_DMA_CH, (void *)&OV7725_EAGLE_DATAPORT, (void *)ov7725_eagle_img_buff, OV7725_EAGLE_PLCK, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);

    DMA_DIS(OV7725_EAGLE_DMA_CH);
    disable_irq(OV7725_EAGLE_INTERRUPT_NUNBERS);                        //关闭PTA的中断
    DMA_IRQ_CLEAN(OV7725_EAGLE_DMA_CH);                                       //清除通道传输中断标志位
    DMA_IRQ_EN(OV7725_EAGLE_DMA_CH);

    port_init(OV7725_EAGLE_PLCK, ALT1 | DMA_FALLING | PULLDOWN );         //PCLK

    port_init(OV7725_EAGLE_VSYNC, ALT1 | IRQ_RISING  | PULLDOWN | PF);     //场中断，上拉，上降沿触发中断，带滤波
...
void ov7725_eagle_vsync(void)
{
    ...
    if(ov7725_eagle_img_flag == IMG_START)                   //需要开始采集图像
    {
        ...
        disable_irq(OV7725_EAGLE_INTERRUPT_NUNBERS);

#if 1

        OV7725_EAGLE_INTERRUPT_ISFR = 1 <<  OV7725_EAGLE_PLCK_CHANNEL;            //清空PCLK标志位
...
#else
        OV7725_EAGLE_INTERRUPT_ISFR = 1 <<  OV7725_EAGLE_PLCK_CHANNEL;            //清空PCLK标志位
        dma_repeat(OV7725_EAGLE_DMA_CH, (void *)&OV7725_EAGLE_DATAPORT, (void *)ov7725_eagle_img_buff,CAMERA_DMA_NUM);
#endif
    }
    else                                        //图像采集错误
    {
...
void ov7725_eagle_get_img()
{
    ...
    {
      ov7725_eagle_img_buff = img_buff2;
      img_switch_flag = 1;
    }
    else
    {
      ov7725_eagle_img_buff = img_buff;
      img_switch_flag = 0; 
    }
